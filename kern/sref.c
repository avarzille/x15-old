...
{
    atomic_or(&weakref->addr, SREF_WEAKREF_DYING);
}

static void
sref_weakref_clear_dying(struct sref_weakref *weakref)
{
    atomic_and(&weakref->addr, SREF_WEAKREF_MASK);
}
...
sref_weakref_kill(struct sref_weakref *weakref)
{
    ...
    oldval = atomic_cas_rel(&weakref->addr, addr, (uintptr_t)NULL);
...
sref_weakref_tryget(struct sref_weakref *weakref)
{
    ...
    do {
        ...
        oldval = atomic_cas_acq(&weakref->addr, addr, newval);
    } while (oldval != addr);
